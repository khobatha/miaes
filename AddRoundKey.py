...
class AddRoundKey(Module):
        def __init__(self,DATA_W,KEY_W):
                #-------------------------------------
                # describe AddRoundKey interface
                #-------------------------------------
                self.NO_DBYTES=int(DATA_W/8)
                self.NO_KBYTES=int(KEY_W/8)
                
                self.valid_text_in=Signal(name="ARK_valid_text_in")# input text data valid flag signal
                self.text_in=[Signal(8,name="ARK_text_in{}".format(s)) for s in range(self.NO_DBYTES)]# input text data signal array
                
                self.valid_rkey_in=Signal(name="ARK_valid_rkey")# input key data valid flag signal
                self.round_key_in=[Signal(8,name="ARK_round_key_in{}".format(s)) for s in range(self.NO_DBYTES)]# input key data signal array
                
                self.valid_data_out=Signal(name="ARK_valid_data_out")# output text data valid flag signal
                self.data_out=[Signal(8,name="ARK_data_out{}".format(s)) for s in range(self.NO_DBYTES)] # output text data signal array
                ...
                #-------------------------------------
                # describe AddRoundKey behavior
                #-------------------------------------
                for b in range(self.NO_DBYTES):
                    self.sync+=[If(self.valid_text_in==1,
                                   If(self.valid_rkey_in==1,
                                        self.data_out[b].eq(self.text_in[b]),# TO DO - FIX XOR OPERATION
                                        self.valid_data_out.eq(1)))]
